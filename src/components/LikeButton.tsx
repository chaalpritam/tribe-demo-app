...
interface LikeButtonProps {
  ...
  /** "default" = tweet card (small, gray/pink horizontal). "reels" =
   *  big white heart on a dark background, count stacked underneath,
   *  red when liked. */
...
export default function LikeButton({
  ...
  if (variant === "reels") {
    return (
      ...
      >
        <svg
          className={`h-7 w-7 transition-colors ${liked ? "text-red-500" : "text-white"}`}
        ...
        {count > 0 && (
          <span className="mt-0.5 text-xs font-semibold">{count}</span>
        )}
      </button>
    );
  }
